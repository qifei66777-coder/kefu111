...
function wbListDeviceChip(dt) {
    var raw = dt == null ? '' : String(dt);
    var s = raw.toLowerCase();
    if (s.indexOf('ios') >= 0 || s.indexOf('iphone') >= 0 || s.indexOf('ipad') >= 0) {
        return { label: 'iOS', cls: 'wb-chip-ios' };
    }
    if (s.indexOf('android') >= 0) {
        return { label: 'Android', cls: 'wb-chip-android' };
    }
    if (raw.trim()) {
        var t = raw.length > 12 ? raw.slice(0, 12) + '…' : raw;
        return { label: t, cls: 'wb-chip-pc' };
    }
    return { label: 'PC', cls: 'wb-chip-pc' };
}
        ...
        success:function(res){
            if(res.code ==0){
                if(res.data){
                    ...
                    if ($("#v_ip_banned_badge").length) {
                        $("#v_ip_banned_badge").hide();
                    }
                    $("#tel").val(res.data.tel);
                    $("#comment").val(res.data.comment);
                    if(res.data.extends && res.data.extends.os!==undefined){
                        $("#login_device").text(res.data.extends.os + ' ' + res.data.extends.browserName);
                    }
                    if ($("#v_device_type").length) {
                        $("#v_device_type").text(res.data.device_type || '—');
                    }
                    if ($("#v_ip_region").length) {
                        $("#v_ip_region").text(res.data.ip_region || '—');
                    }
                    if ($("#v_qr_remark").length) {
                        $("#v_qr_remark").text(res.data.qr_remark ? res.data.qr_remark : '—');
                    }
                    if ($("#v_scan_time").length) {
                        var sc = res.data.last_scan_time != null ? res.data.last_scan_time : res.data.scan_time;
                        if (sc === undefined || sc === null || sc === '') {
                            $("#v_scan_time").text('—');
                        } else if (/^\d+$/.test(String(sc).trim())) {
                            var n = parseInt(sc, 10);
                            $("#v_scan_time").text(n > 946684800 ? new Date(n * 1000).toLocaleString() : String(sc));
                        } else {
                            $("#v_scan_time").text(String(sc));
                        }
                    }
                    if (typeof window.wbVisiterHead === 'function') {
                        window.wbVisiterHead(res.data);
                    }
        ...
        success: function (res) {
            ...
            if (res.code == 0) {
                ...
                $.each(data, function (k, v) {
                    ...
                    var stv = v.state ? String(v.state) : '';
                    var dchip = wbListDeviceChip(v.device_type);
                    var regv = (v.ip_region != null && String(v.ip_region).trim()) ? String(v.ip_region).trim() : '—';
                    b='<i class="layui-icon myicon hide" title="删除" style="font_weight:blod" onclick="cut(' + "'" + v.visiter_id + "'" + ')">&#x1006;</i>';
                    a += '<div id="v' + v.channel + '" class="visiter '+isclick+'" data-wb-unread="'+uraw+'" data-wb-state="'+stv.replace(/"/g,'')+'" onmouseover="showcut(this)" onmouseout="hidecut(this)">'+b+'<span id="c' +v.channel + '" class="notice-icon '+ishide+'">'+v.count+'</span>';
                    a += "<div class='visit_content' onclick='choose(" +v.vid+ ")'><img class='am-radius v-avatar "+isicon_gray+"' id='img" + v.channel + "' src='" + v.avatar + "'  width='50px'>"+name+"<span class='c_time'>" + v.timestamp + "</span><div id='msg" + v.channel + "' class='newmsg'>"+v.content+"</div>";
                    a += "<div class='wb-visiter-meta'><span class='wb-chip "+dchip.cls+"'>"+wbEscHtml(dchip.label)+"</span><span class='wb-chip wb-chip-loc'>"+wbEscHtml(regv)+"</span></div></div></div>";
                ...
                $.each(data, function (k, v) {
                    ...
                    if (v.direction == 'to_visiter') {
                        ...
                        if (v.content && v.content.indexOf('wolive-rich-reply') !== -1) {
                            str += "<div class=\"chat-msg-rich-wrap\">" + v.content + "</div>&nbsp;&nbsp;<span onclick='revoke("+v.cid+",1);' class='revoke-text'>(撤销)</span>";
                        } else {
                        ...
                        str += "</div>";
                        str += "<pre id='cid"+v.cid+"' class='noredcustomer'>" + name + "</pre>";
                        str += "</div>";
                        str += "</li>";
                    } else{
