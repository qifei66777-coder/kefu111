/**
 * AraleQRCode - Standalone browser build
 * Based on arale-qrcode@3.0.5 (https://github.com/aralejs/qrcode)
 * MIT License
 */
(function () {
    'use strict';
    ...
    QRCodeAlg.prototype = {
        ...
        setupTimingPattern: function () {
            for (var r = 8; r < this.moduleCount - 8; r++) {
                ...
                if (this.modules[6][r] != null) continue;
        ...
        getLostPoint: function (qrCode) {
            ...
            for (var row = 0; row < moduleCount; row++) {
                ...
                for (var col = 0; col < moduleCount; col++) {
                    ...
                    if (col < moduleCount - 6) {
                        if (current && !qrCode.modules[row][col + 1] && qrCode.modules[row][col + 2] && qrCode.modules[row][col + 3] && qrCode.modules[row][col + 4] && !qrCode.modules[row][col + 5] && qrCode.modules[row][col + 6]) {
                            if (col < moduleCount - 10) {
                                if (qrCode.modules[row][col + 7] && qrCode.modules[row][col + 8] && qrCode.modules[row][col + 9] && qrCode.modules[row][col + 10]) { lostPoint += 40; }
                            } else if (col > 3) {
                                if (qrCode.modules[row][col - 1] && qrCode.modules[row][col - 2] && qrCode.modules[row][col - 3] && qrCode.modules[row][col - 4]) { lostPoint += 40; }
                            }
                        }
                    }
                    ...
                    if (head ^ current) { sameCount++; } else {
                        head = current;
                    ...
                    if (row < moduleCount - 6) {
                        if (current && !qrCode.modules[row + 1][col] && qrCode.modules[row + 2][col] && qrCode.modules[row + 3][col] && qrCode.modules[row + 4][col] && !qrCode.modules[row + 5][col] && qrCode.modules[row + 6][col]) {
                            if (row < moduleCount - 10) {
                                if (qrCode.modules[row + 7][col] && qrCode.modules[row + 8][col] && qrCode.modules[row + 9][col] && qrCode.modules[row + 10][col]) { lostPoint += 40; }
                            } else if (row > 3) {
                                if (qrCode.modules[row - 1][col] && qrCode.modules[row - 2][col] && qrCode.modules[row - 3][col] && qrCode.modules[row - 4][col]) { lostPoint += 40; }
                            }
                        }
                    }
                    if (head ^ current) { sameCount++; } else {
    ...
    QRPolynomial.prototype = {
        ...
        mod: function (e) {
            ...
            if (tl - el < 0) return this;
            var num = new Array(tl);
            for (var i = 0; i < tl; i++) { num[i] = this.get(i); }
            while (num.length >= el) {
    ...
    QRCodeAlg.prototype.getRightType = function () {
        for (var typeNumber = 1; typeNumber < 41; typeNumber++) {
            ...
            if (rsBlock == undefined) throw new Error("bad rs block @ typeNumber:" + typeNumber + "/errorCorrectLevel:" + this.errorCorrectLevel);
    ...
    QRBitBuffer.prototype = {
        ...
        put: function (num, length) {
            for (var i = 0; i < length; i++) { this.putBit(num >>> length - i - 1 & 1); }
        },
        putBit: function (bit) {
    ...
    var AraleQRCode = function (opt) {
        ...
        for (var i = 0, l = qrcodeAlgObjCache.length; i < l; i++) {
            if (qrcodeAlgObjCache[i].text == this.options.text && qrcodeAlgObjCache[i].correctLevel == this.options.correctLevel) {
        ...
        if (this.options.render) {
            switch (this.options.render) {
                case 'canvas': return this.createCanvas(qrCodeAlg);
                case 'table': return this.createTable(qrCodeAlg);
                case 'svg': return this.createSVG(qrCodeAlg);
                default: return this.createDefault(qrCodeAlg);
            }
        }
        return this.createDefault(qrCodeAlg);
    };

    extend(AraleQRCode.prototype, {
        ...
        createCanvas: function (qrCodeAlg) {
            ...
            var loadImage = function (url, callback) {
                var img = new Image();
                img.src = url;
                img.onload = function () { callback(this); img.onload = null; };
            };
            ...
            for (var row = 0; row < count; row++) {
                for (var col = 0; col < count; col++) {
                    ...
                    var foreground = getForeGround({ row: row, col: col, count: count, options: options });
        ...
        createTable: function (qrCodeAlg) {
            ...
            if (tileW <= 0) tileW = count < 80 ? 2 : 1;
            if (tileH <= 0) tileH = count < 80 ? 2 : 1;
            var s = [];
        ...
        createSVG: function (qrCodeAlg) {
            ...
            for (var row = 0; row < count; row++) {
                for (var col = 0; col < count; col++) {
                    ...
                    rect.setAttribute('fill', qrCodeAlg.modules[row][col] ? foreground : options.background);
    ...
    window.AraleQRCode = AraleQRCode;
})();
