...
function showinfo(data,group) {
    ...
    value += '<div class="info_form_item" style="display:none;"><label>网站id</label><span><span> '+data.business_id+' </span><input type="text" name="id" class="hide" value=' + '"' + data.service_id + '"' + '></span></div>';
    value += '<div class="info_form_item" style="display:none;"><label>客服分组</label><span><select id="classification" name="groupid">';
    if(data.groupid == 0){
        value +='<option value="0" selected="selected">通用客服</option>';
    }
    $.each(group,function(k,v){
