...
class Set extends Base
{
    ...
    public function chats()
    {
        ...
        $arr['content'] = $this->request->post('content');
        ...
        try {
            ...
            return ['code' => 0, 'msg' => 'success',"cid" => $arr['cid']];
        } catch (Exception $e) {
            $error = $e->getMessage();
...
unset($admin['password']);
    ...
    public function getchats()
    {
        ...
        $data=Db::query("select v.* from ".config('database.prefix')."visiter as v LEFT JOIN ".config('database.prefix')."queue as q ON v.visiter_id=q.visiter_id where v.business_id=:business_id and q.business_id=:business_id2 and q.service_id=:service_id and q.state='normal' order by v.istop desc,q.lastpost desc,q.timestamp desc limit 100",['business_id'=>$login['business_id'],'business_id2'=>$login['business_id'],'service_id'=>$login['service_id']]);
...
//        return $arr;
    ...
    public function delete21()
    {
        $post = $this->request->post();
        $login = session('Msg');

        $result = model('queue')->where('visiter_id', $post['visiter_id'])->where('business_id', $login['business_id'])->delete();
    ...
    public function setofflinefirst2()
    {
        $post = $this->request->post();
        $type = $post['type'];
        $result = model('service')->where('service_id', $post['id'])->update(['groupid' => $type]);

        $arr = ['code' => 0, 'msg' => 'success', 'data' => $result];
        return $arr;
    }
    public function setofflinefirst3()
    {
        $post = $this->request->post();
        $type = $post['type']=='groupid' ? 'groupid':10000;
        $result = model('service')->where('service_id', $post['id'])->update(['groupid' => $type]);
