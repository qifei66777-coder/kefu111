...
hook('mobileindexhook',array_merge($arr,$arr2));
        ...
        $visMeta = Db::name('visiter')->field('qr_remark,device_type,ip_region')->where(['visiter_id' => $visiter_id, 'business_id' => $business_id])->find();
        $tagSource = ($visMeta && trim((string) $visMeta['qr_remark']) !== '') ? htmlspecialchars(trim((string) $visMeta['qr_remark']), ENT_QUOTES, 'UTF-8') : '—';
        $tagDevice = ($visMeta && trim((string) $visMeta['device_type']) !== '') ? htmlspecialchars(trim((string) $visMeta['device_type']), ENT_QUOTES, 'UTF-8') : '—';
        $tagRegion = ($visMeta && trim((string) $visMeta['ip_region']) !== '') ? htmlspecialchars(trim((string) $visMeta['ip_region']), ENT_QUOTES, 'UTF-8') : '—';
        $this->assign('tag_source', $tagSource);
        $this->assign('tag_device', $tagDevice);
        $this->assign('tag_region', $tagRegion);
