...
//  随机生成访客id
  ...
  public function remove_emoji($nickname) {
    ...
    /**
     * 校验 IPv4/IPv6 地址字符串（不信任空值、过滤非法格式）
     *
     * @param string $ip
     * @return bool
     */
    public function isValidClientIp($ip)
    {
        if ($ip === null || $ip === '') {
            return false;
        }
    ...
    /**
     * 归一化代理头里的 IP（去空白，剥离 IPv4:端口）
     *
     * @param string $ip
     * @return string
     */
    protected function normalizeIpCandidate($ip)
    {
        $ip = trim((string) $ip);
        if ($ip === '') {
            return '';
    ...
    /**
     * 获取客户端 IP。优先级：CF-Connecting-IP → X-Real-IP → X-Forwarded-For 首个有效值 → REMOTE_ADDR
     *
     * @return string
     */
    public function getClientIp()
    {
        ...
        foreach ($headers as $h) {
            ...
            $ip = $this->normalizeIpCandidate($_SERVER[$h]);
            if ($ip !== '' && $this->isValidClientIp($ip)) {
                return $ip;
            }
        }
        ...
        if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
            ...
            foreach ($parts as $p) {
                $ip = $this->normalizeIpCandidate($p);
                if ($ip !== '' && $this->isValidClientIp($ip)) {
                    return $ip;
                }
            }
        ...
        if (!empty($_SERVER['REMOTE_ADDR'])) {
            $ip = $this->normalizeIpCandidate($_SERVER['REMOTE_ADDR']);
            if ($ip !== '' && $this->isValidClientIp($ip)) {
                return $ip;
            }
        }
    ...
    /**
     * 终端类型：iOS（含 iphone/ipad/ipod）、Android、PC（非移动）、Other（无法归类）
     *
     * @return string
     */
    public function detectDeviceCategory()
    {
        ...
        $l = strtolower($ua);
        if (preg_match('/iphone|ipad|ipod/', $l)) {
            return 'iOS';
        }
        if (strpos($l, 'android') !== false) {
            return 'Android';
        }
        if ($this->isMobile()) {
            return 'Other';
