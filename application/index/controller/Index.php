...
//                        不相等则更新
    ...
    /**
     * 二维码渠道进入：校验渠道与令牌后跳转至原 H5/PC 对话入口（/index/index?code=...）。
     *
     * @return void
     */
    public function kfchat()
    {
        $ch = intval($this->request->param('ch', 0));
        $token = trim($this->request->param('token', ''));
        if ($ch <= 0 || $token === '') {
            return $this->kfchatNotice('invalid_link');
        }

        $row = Db::name('qr_channels')->where('id', $ch)->find();
        if (!$row) {
            return $this->kfchatNotice('channel_missing');
        }
        if (!hash_equals((string) $row['channel_token'], $token)) {
            return $this->kfchatNotice('token_mismatch');
        }
        $channel = $row;

        if (intval($channel['status']) === -1) {
            return $this->kfchatNotice('channel_removed');
        }
        if (intval($channel['status']) !== 1) {
            return $this->kfchatNotice('channel_disabled');
        }

        $business_id = intval($channel['business_id']);
        $business = model('business')->where('id', $business_id)->find();
        if (!$business) {
            return $this->kfchatNotice('merchant_off');
        }
        if (isset($business['state']) && $business['state'] == 'close') {
            return $this->kfchatNotice('merchant_off');
        }

        $templateId = intval($channel['template_id']);
        if ($templateId > 0) {
            $tpl = Db::name('qr_templates')
                ->where('id', $templateId)
                ->where('business_id', $business_id)
                ->where('is_delete', 0)
                ->find();
            if (!$tpl || intval($tpl['status']) !== 1) {
                return $this->kfchatNotice('template_off');
            }
        }

        $common = new Common();
        $ip = $common->getClientIp();
        $blocked = Db::name('ip_blacklist')
            ->where('business_id', $business_id)
            ->where('ip', $ip)
            ->where('status', 1)
            ->find();
        if ($blocked) {
            return $this->kfchatNotice('ip_blocked');
        }

        $deviceType = $common->detectDeviceCategory();
        $ipRegion = '未知';
        if ($ip !== '' && $ip !== '0.0.0.0' && $common->isValidClientIp($ip)) {
            try {
                $loc = \app\admin\iplocation\Ip::find($ip);
                if (is_array($loc) && isset($loc[0])) {
                    $ipRegion = $loc[0]
                        . (isset($loc[1]) ? $loc[1] : '')
                        . (isset($loc[2]) ? $loc[2] : '')
                        . (isset($loc[3]) ? $loc[3] : '');
                    $ipRegion = trim((string) $ipRegion);
                    if ($ipRegion === '' || $ipRegion === 'N/A') {
                        $ipRegion = '未知';
                    }
                }
            } catch (\Throwable $e) {
        ...
        if ($visiter_name === '') {
            $visiter_name = '游客' . mb_substr($visiter_id, 0, 12, 'UTF-8');
        }

        $channelHex = bin2hex($visiter_id . '/' . $business_id);
        $visiterRow = Db::name('visiter')->where(['visiter_id' => $visiter_id, 'business_id' => $business_id])->find();
        if ($visiterRow) {
            Db::name('visiter')->where('vid', $visiterRow['vid'])->update([
        ...
        $now = time();
        Db::name('qr_scan_logs')->insert([
            'business_id'  => $business_id,
            'service_id'   => intval($channel['service_id']),
            ...
            'ip_region'    => $ipRegion,
            'device_type'  => $deviceType,
            'user_agent'   => mb_substr((string) $this->request->server('HTTP_USER_AGENT'), 0, 1000, 'UTF-8'),
            'from_url'     => isset($_SERVER['HTTP_REFERER']) ? mb_substr((string) $_SERVER['HTTP_REFERER'], 0, 1000, 'UTF-8') : '',
            'scan_time'    => $now,
        ]);
        ...
        $groupid = 0;
        if ($service && isset($service['groupid']) && $service['groupid'] !== '' && $service['groupid'] !== null) {
            $groupid = intval($service['groupid']);
        }

        if ($service) {
            QrWelcome::tryCommitOnKfchat($business->toArray(), $ch, $visiter_id, $service);
        }

        $payload = [
            'business_id'  => $business_id,
            'groupid'      => $groupid,
            'special'      => intval($channel['service_id']),
            'visiter_id'   => $visiter_id,
            'visiter_name' => $visiter_name,
            'avatar'       => '',
            'theme'        => '05202d',
        ];
        $newstr = $common->encrypt(http_build_query($payload), 'E', YMWL_SALT);

        $domain_landing = BASEROOT;
        if (!empty($business['domain_landing'])) {
            $domain_landing = $business['domain_landing'];
        }
        hook('homejumpbeforehook', array_merge($payload, ['code' => $newstr]));
        $this->redirect($domain_landing . '/index/index?code=' . $newstr);
    }
    ...
    public function index()
    {
        ...
        $theme=isset($arr2['theme'])?$arr2['theme']:'05202d';
        ...
        $service = model('queue')->where('business_id', $business_id)->where('visiter_id',$visiter_id)->where('state', 'normal')->find();
        $service_id = $service ? $service['service_id'] : null;
        //===
        $data = model('chats')->where(['service_id' => $service_id,'visiter_id' => $visiter_id,'direction' => 'to_visiter','state' => 'unread', 'business_id' => $business_id])->select();
        $cids = [];
       
        foreach ($data as $value){
            model('chats')->where('cid', $value['cid'])->update(['state' => 'readed']);
            $cids[]=$value['cid'];
        }
        
        //$channel = bin2hex($post['visiter_id'] . '/' . $login['business_id']);
        if(!empty($cids) && !empty($service_id)){
            $pusher = SinglePusher::getinstance();
            $pusher->trigger('kefu' . $service_id, 'check-event', array('message' => $cids));
        }
        ...
        $flash = QrWelcome::pullFlashForChat($business_id, $visiter_id);
        $this->assign(
            'scan_welcome_json',
            $flash
                ? json_encode(
                    ['content' => $flash['content'], 'avatar' => $flash['avatar']],
...
//    技术支持QQ1500203929
     ...
     * @return mixed
     */
    private function kfchatNotice($reason)
    {
        $map = [
            'invalid_link' => [
                'title' => '链接无效',
                'hint' => '二维码参数不完整，请重新扫描客服提供的二维码。',
            ],
            'channel_missing' => [
                'title' => '二维码已失效',
                'hint' => '找不到对应的会话入口，请让客服重新生成二维码后再试。',
            ],
            'token_mismatch' => [
                'title' => '链接无法验证',
                'hint' => '安全校验未通过，请重新扫描最新二维码。',
            ],
            'channel_disabled' => [
                'title' => '二维码已停用',
                'hint' => '该会话渠道已暂停使用，请联系客服获取新的二维码。',
            ],
            'channel_removed' => [
                'title' => '二维码已失效',
                'hint' => '该会话入口已关闭且无法恢复，请让客服重新生成二维码。',
            ],
            'merchant_off' => [
                'title' => '客服暂不可用',
                'hint' => '商户客服当前无法接待，请稍后再试。',
            ],
            'template_off' => [
                'title' => '二维码模板不可用',
                'hint' => '展示模板已被停用，请让客服重新创建会话。',
            ],
            'ip_blocked' => [
                'title' => '暂时无法接入',
        ...
        $this->assign('hint', $m['hint']);

        return $this->fetch('kfchat_notice');
    }
